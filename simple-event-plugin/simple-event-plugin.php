...
Version: 1.9
Author: Miaoulis N
Text Domain: simple-event-plugin
...
function sep_load_textdomain() {
    load_plugin_textdomain('simple-event-plugin', false, dirname(plugin_basename(__FILE__)) . '/languages/');
}
add_action('plugins_loaded', 'sep_load_textdomain');

// Register Custom Post Type for Events
function sep_register_event_post_type() {
    $labels = array(
        'name' => __('Events', 'simple-event-plugin'),
        'singular_name' => __('Event', 'simple-event-plugin'),
        'menu_name' => __('Events', 'simple-event-plugin'),
        'add_new' => __('Add New Event', 'simple-event-plugin'),
        'add_new_item' => __('Add New Event', 'simple-event-plugin'),
        'edit_item' => __('Edit Event', 'simple-event-plugin'),
        'new_item' => __('New Event', 'simple-event-plugin'),
        'view_item' => __('View Event', 'simple-event-plugin'),
        'search_items' => __('Search Events', 'simple-event-plugin'),
        'not_found' => __('No events found', 'simple-event-plugin'),
        'not_found_in_trash' => __('No events found in Trash', 'simple-event-plugin'),
    );
...
function sep_register_event_category() {
    $labels = array(
        'name' => __('Event Categories', 'simple-event-plugin'),
        'singular_name' => __('Event Category', 'simple-event-plugin'),
        'search_items' => __('Search Event Categories', 'simple-event-plugin'),
        'all_items' => __('All Event Categories', 'simple-event-plugin'),
        'edit_item' => __('Edit Event Category', 'simple-event-plugin'),
        'update_item' => __('Update Event Category', 'simple-event-plugin'),
        'add_new_item' => __('Add New Event Category', 'simple-event-plugin'),
        'new_item_name' => __('New Event Category Name', 'simple-event-plugin'),
        'menu_name' => __('Event Categories', 'simple-event-plugin'),
    );
...
function sep_add_event_meta_boxes() {
    add_meta_box(
        'sep_event_details',
        __('Event Details', 'simple-event-plugin'),
...
function sep_event_details_callback($post) {
    ...
    ?>
    <p>
        <label for="sep_event_date"><?php _e('Event Date:', 'simple-event-plugin'); ?></label><br>
        <input type="date" id="sep_event_date" name="sep_event_date" value="<?php echo esc_attr($event_date); ?>" />
    </p>
    <p>
        <label for="sep_event_time"><?php _e('Event Time:', 'simple-event-plugin'); ?></label><br>
        <input type="time" id="sep_event_time" name="sep_event_time" value="<?php echo esc_attr($event_time); ?>" />
    </p>
    <p>
        <label for="sep_event_location"><?php _e('Event Location:', 'simple-event-plugin'); ?></label><br>
        <input type="text" id="sep_event_location" name="sep_event_location" value="<?php echo esc_attr($event_location); ?>" style="width: 100%;" />
    </p>
    <p>
        <label for="sep_event_speaker"><?php _e('Speaker (Optional):', 'simple-event-plugin'); ?></label><br>
...
function sep_display_events_shortcode($atts) {
    ...
    $display_mode = get_option('sep_display_mode', 'latest');
    $max_cards = absint(get_option('sep_max_cards', 3));
    ...
    $show_attendees = get_option('sep_show_attendees', 'yes');
    ...
    if ($atts['view'] === 'calendar') {
        ...
        if ($events->have_posts()) {
            $output .= '<table>';
            $output .= '<thead><tr><th>' . __('Date', 'simple-event-plugin') . '</th><th>' . __('Event', 'simple-event-plugin') . '</th><th>' . __('Status', 'simple-event-plugin') . '</th></tr></thead><tbody>';
            while ($events->have_posts()) {
                ...
                $status = ($event_datetime > $current_time) ? __('Upcoming', 'simple-event-plugin') : (($event_datetime <= $current_time && $event_datetime + 3600 >= $current_time) ? __('Live', 'simple-event-plugin') : __('Past', 'simple-event-plugin'));
            ...
            $output .= '<p>' . __('No events found.', 'simple-event-plugin') . '</p>';
        }
        $output .= '</div>';
    } else {
        ...
        if ($events->have_posts()) {
            ...
            while ($events->have_posts() && $count < $max_cards) {
                ...
                $output .= '<p><strong>' . __('Location:', 'simple-event-plugin') . '</strong> ' . esc_html($event_location) . '</p>';
                $output .= '<p><strong>' . __('Status:', 'simple-event-plugin') . '</strong> <span class="sep-event-status sep-status-' . strtolower($status) . '">' . esc_html($status) . '</span></p>';
                if ($show_attendees === 'yes') {
                    $output .= '<p><strong>' . __('Attendees:', 'simple-event-plugin') . '</strong> ' . esc_html($rsvp_count) . '</p>';
                }
                if ($show_category === 'yes' && !empty($category_names)) {
                    $output .= '<p><strong>' . __('Category:', 'simple-event-plugin') . '</strong> ' . esc_html(implode(', ', $category_names)) . '</p>';
                }
                if ($show_speaker === 'yes' && !empty($event_speaker)) {
                    $output .= '<p><strong>' . __('Speaker:', 'simple-event-plugin') . '</strong> ' . esc_html($event_speaker) . '</p>';
                }
                ...
                $output .= '<button type="submit" name="sep_rsvp_submit" class="' . ($has_rsvped ? 'sep-rsvp-cancel' : '') . '">' . ($has_rsvped ? __('Cancel RSVP', 'simple-event-plugin') : __('RSVP', 'simple-event-plugin')) . '</button>';
                $output .= '</form>';
    ...
    $carousel_js = "
        document.addEventListener('DOMContentLoaded', function() {
            ...
            carousels.forEach(carousel => {
                ...
                let scrollAmount = 0;
                const scrollStep = inner.querySelector('.sep-event') ? inner.querySelector('.sep-event').offsetWidth + 20 : 320;
                setInterval(() => {
...
function sep_settings_menu() {
    add_options_page(
        __('Simple Event Plugin Settings', 'simple-event-plugin'),
        __('Event Settings', 'simple-event-plugin'),
        'manage_options',
        'sep-settings',
        'sep_settings_page'
    );
...
function sep_register_settings() {
    ...
    register_setting('sep_settings_group', 'sep_show_attendees');
...
function sep_settings_page() {
    ?>
    <div class="wrap">
        <h1><?php _e('Simple Event Plugin Settings', 'simple-event-plugin'); ?></h1>
        <form method="post" action="options.php">
            <?php settings_fields('sep_settings_group'); ?>
            <?php do_settings_sections('sep_settings_group'); ?>
            <table class="form-table">
                <tr>
                    <th><label for="sep_primary_color"><?php _e('Primary Color (Titles, Accents)', 'simple-event-plugin'); ?></label></th>
                    <td><input type="color" name="sep_primary_color" value="<?php echo esc_attr(get_option('sep_primary_color', '#0073aa')); ?>" /></td>
                </tr>
                <tr>
                    <th><label for="sep_secondary_color"><?php _e('Secondary Color (Card Background)', 'simple-event-plugin'); ?></label></th>
                    <td><input type="color" name="sep_secondary_color" value="<?php echo esc_attr(get_option('sep_secondary_color', '#f5f5f5')); ?>" /></td>
                </tr>
                <tr>
                    <th><label for="sep_text_color"><?php _e('Text Color', 'simple-event-plugin'); ?></label></th>
                    <td><input type="color" name="sep_text_color" value="<?php echo esc_attr(get_option('sep_text_color', '#333')); ?>" /></td>
                </tr>
                <tr>
                    <th><label for="sep_accent_color"><?php _e('Accent Color (Hover)', 'simple-event-plugin'); ?></label></th>
                    <td><input type="color" name="sep_accent_color" value="<?php echo esc_attr(get_option('sep_accent_color', '#00aaff')); ?>" /></td>
                </tr>
                <tr>
                    <th><label for="sep_show_category"><?php _e('Show Event Category', 'simple-event-plugin'); ?></label></th>
                    <td>
                        <select name="sep_show_category" id="sep_show_category">
                            <option value="yes" <?php selected(get_option('sep_show_category', 'yes'), 'yes'); ?>><?php _e('Yes', 'simple-event-plugin'); ?></option>
                            <option value="no" <?php selected(get_option('sep_show_category', 'yes'), 'no'); ?>><?php _e('No', 'simple-event-plugin'); ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
                    <th><label for="sep_show_speaker"><?php _e('Show Event Speaker', 'simple-event-plugin'); ?></label></th>
                    <td>
                        <select name="sep_show_speaker" id="sep_show_speaker">
                            <option value="yes" <?php selected(get_option('sep_show_speaker', 'yes'), 'yes'); ?>><?php _e('Yes', 'simple-event-plugin'); ?></option>
                            <option value="no" <?php selected(get_option('sep_show_speaker', 'yes'), 'no'); ?>><?php _e('No', 'simple-event-plugin'); ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
                    <th><label for="sep_show_description"><?php _e('Show Event Description', 'simple-event-plugin'); ?></label></th>
                    <td>
                        <select name="sep_show_description" id="sep_show_description">
                            <option value="yes" <?php selected(get_option('sep_show_description', 'no'), 'yes'); ?>><?php _e('Yes', 'simple-event-plugin'); ?></option>
                            <option value="no" <?php selected(get_option('sep_show_description', 'no'), 'no'); ?>><?php _e('No', 'simple-event-plugin'); ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
                    <th><label for="sep_show_attendees"><?php _e('Show Attendees Count', 'simple-event-plugin'); ?></label></th>
                    <td>
                        <select name="sep_show_attendees" id="sep_show_attendees">
                            <option value="yes" <?php selected(get_option('sep_show_attendees', 'yes'), 'yes'); ?>><?php _e('Yes', 'simple-event-plugin'); ?></option>
                            <option value="no" <?php selected(get_option('sep_show_attendees', 'yes'), 'no'); ?>><?php _e('No', 'simple-event-plugin'); ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
                    <th><label for="sep_max_cards"><?php _e('Max Number of Cards (Desktop)', 'simple-event-plugin'); ?></label></th>
                    <td>
                        <input type="number" name="sep_max_cards" id="sep_max_cards" value="<?php echo esc_attr(get_option('sep_max_cards', 3)); ?>" min="1" step="1" />
                        <p class="description"><?php _e('Set the maximum number of event cards to display on desktop. Tablet will show up to 2, and mobile up to 1.', 'simple-event-plugin'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th><label for="sep_display_mode"><?php _e('Display Mode', 'simple-event-plugin'); ?></label></th>
                    <td>
                        <select name="sep_display_mode" id="sep_display_mode">
                            <option value="latest" <?php selected(get_option('sep_display_mode', 'latest'), 'latest'); ?>><?php _e('Max 3 Latest', 'simple-event-plugin'); ?></option>
                            <option value="oldest" <?php selected(get_option('sep_display_mode', 'latest'), 'oldest'); ?>><?php _e('Max 3 Oldest', 'simple-event-plugin'); ?></option>
                            <option value="quarter" <?php selected(get_option('sep_display_mode', 'latest'), 'quarter'); ?>><?php _e('Current Quarter', 'simple-event-plugin'); ?></option>
                        </select>
                        <p class="description"><?php _e('Choose which events to display: latest, oldest, or those in the current quarter.', 'simple-event-plugin'); ?></p>
                    </td>
                </tr>
            </table>
            <h2><?php _e('Shortcode Reference', 'simple-event-plugin'); ?></h2>
            <p><?php _e('Use the following shortcodes to display events:', 'simple-event-plugin'); ?></p>
            <ul>
                <li><code>[display_events]</code> - <?php _e('Display events in a carousel based on display mode.', 'simple-event-plugin'); ?></li>
                <li><code>[display_events category="slug"]</code> - <?php _e('Filter by category.', 'simple-event-plugin'); ?></li>
                <li><code>[display_events event_id="123"]</code> - <?php _e('Display a single event by ID.', 'simple-event-plugin'); ?></li>
                <li><code>[display_events view="calendar"]</code> - <?php _e('Display all events in a calendar view.', 'simple-event-plugin'); ?></li>
                <li><code>[display_latest_category_event category="workshops"]</code> - <?php _e('Display the latest event from the "workshops" category.', 'simple-event-plugin'); ?></li>
            </ul>
            <?php submit_button(); ?>
        </form>
    </div>
...
// Shortcode to Display the Latest Event from a Category
function sep_display_latest_category_event_shortcode($atts) {
    $atts = shortcode_atts(array(
        'category' => '', // Category slug is required
    ), $atts, 'display_latest_category_event');

    // Check if category is provided
    if (empty($atts['category'])) {
        return '<p>' . __('Please specify an event category.', 'simple-event-plugin') . '</p>';
    }
    ...
    $output = '<div class="sep-events-list">';
    if ($events->have_posts()) {
        while ($events->have_posts()) {
            $events->the_post();
            $event_date = get_post_meta(get_the_ID(), '_sep_event_date', true);
        ...
        $output .= '<p>' . __('No events found in this category.', 'simple-event-plugin') . '</p>';
    }
    $output .= '</div>';
...
add_shortcode('display_latest_category_event', 'sep_display_latest_category_event_shortcode');
